...
<?php
	if(!empty($this->manage)) {
		...
		echo $this->loadHkLayout('quantity', array(
			...
			'force_input' => true,
			'extra_data' => 'data-hk-product-name="'.$this->escape(strip_tags($product->product_name)).'" onkeypress="if(event.keyCode==13 && window.cartMgr.checkQuantity(this)){ window.hikashop.submitform(\'apply\',\'hikashop_show_cart_form\'); } return false;"',
		));
	} else {
?>
